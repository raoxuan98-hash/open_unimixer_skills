...
class KroneckerMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        output =  x_global.reshape(B, N * K) # + 0.3 * x
        return output
...
class UniMixerLiteLayer(nn.Module):
    ...
    Same structure as RankMixerLayer: mixing -> norm1 -> ffn -> norm2.
    """
    ...
    def forward(self, src):
        ...
        mixing_h = src.reshape(B, T * D)
        mixing_h = self.mixer(mixing_h)
        mixing_h = mixing_h.view(B, T, D)

        mixed = self.norm1(src + mixing_h)

        # Token-specific SwishGLU
        ffn_out = self.ffn(mixed)
        out = self.norm2(mixed + ffn_out)
        return out
