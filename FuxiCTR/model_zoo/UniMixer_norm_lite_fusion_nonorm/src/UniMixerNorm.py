...
class UniMixerLiteLayer(nn.Module):
    ...
    def __init__(self, seq_len, d_model, block_size, basis_num, d_ff,
        ...
        self.ffn = NPerTokenSwishGLU(num_tokens=seq_len, in_dim=d_model, hidden_dim=d_ff, dropout=dropout)

        # self.norm_y = NormalizedResidualUpdate(
        #     num_tokens=d_model,
        #     in_dim=d_model,
        #     alpha_init=0.05,
        ...
        self.mix_residual = NormalizedResidualUpdate(
            num_tokens=seq_len,
            in_dim=d_model,
            alpha_init=0.05,
            gate_type="token_wise",
        ...
        self.ffn_residual_x = NormalizedResidualUpdate(
            num_tokens=seq_len,
            in_dim=d_model,
            alpha_init=0.05,
            gate_type="token_wise",
        ...
        self.ffn_residual_y = NormalizedResidualUpdate(
            num_tokens=seq_len,
            in_dim=d_model,
            alpha_init=0.05,
            gate_type="token_wise",
    ...
    def forward(self, x, y):
        ...
        mixing_input = x + y
        mixing_input = F.normalize(mixing_input, p=2, dim=-1)
        ...
        mixing_h = F.normalize(mixing_h, p=2, dim=-1)

        pffn_input = self.mix_residual(mixing_input, mixing_h)

        # NPerTokenSwishGLU
        ffn_output = self.ffn(pffn_input)

        final_x = self.ffn_residual_x(x, ffn_output)
        final_y = self.ffn_residual_y(y, ffn_output)
