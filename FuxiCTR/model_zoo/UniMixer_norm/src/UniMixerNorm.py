...
class UniMixerBlock(nn.Module):
    ...
    def __init__(self, total_dim, block_size, token_num_after_reshape,
        ...
        # 全局 FFN (无门控 basis weighted)
        # 输入是转置后的 [B, K, N]，所以 num_tokens=K, in_dim=N
        self.global_ffn = NPerTokenNoGLU_Basis(
            num_tokens=self.K,
            in_dim=self.N,
            hidden_dim=token_num_after_reshape,
            basis_num=token_ffn_basis_num,
            dropout=dropout,
        )
    ...
    def forward(self, x):
        ...
        # 全局 FFN
        x_global = self.global_ffn(x_local_trans)  # [B, K, N]
...
class UniMixerNormLayer(nn.Module):
    ...
    def __init__(self, seq_len, d_model, d_ff, dropout=0.1,
        ...
        if block_size is not None:
            # 使用 UniMixerBlock 进行 token 重新划分和局部变换
            ...
            # input is transposed to [B, D, L],
            # so we swap seq_len and d_model for the mixer.
            # num_tokens = d_model (feature dim becomes positions),
            # in_dim     = seq_len (original seq_len becomes features per position)
            if use_basis:
                self.seq_mixer = NPerTokenSwishGLU_Basis(
                    num_tokens=d_model,
                    in_dim=seq_len,
                ...
                self.seq_mixer = NPerTokenSwishGLU(
                    num_tokens=d_model,
                    in_dim=seq_len,
                    hidden_dim=seq_len * 10,
                    dropout=dropout,
