...
class NPerTokenNoGLU_Basis(nn.Module):
    """
    基于基函数加权组合的无门控归一化 FFN。

    对应 unimixer_norm_tf.py 中的 normalized_ffn_no_glu_basis_weighted。
    每个 token 通过独立的组合系数从共享的基函数矩阵中构建个性化权重。
    ...
    def __init__(self, num_tokens, in_dim, hidden_dim, basis_num, dropout=0.0):
        super(NPerTokenNoGLU_Basis, self).__init__()
        self.num_tokens = num_tokens
        self.in_dim = in_dim
        self.hidden_dim = hidden_dim
        self.basis_num = basis_num

        # 基础权重矩阵 (两组，对应无门控 FFN 的 w1 和 w2)
        self.W1_basis = nn.Parameter(torch.empty(basis_num, in_dim, hidden_dim))
        self.W2_basis = nn.Parameter(torch.empty(basis_num, hidden_dim, in_dim))
        ...
        # scale: 对应 ngpt_scale
        self.scale = NGPTScale((1, hidden_dim), scale_init=1.0,
                               scale_alpha=math.sqrt(in_dim))
    ...
    def _reset_parameters(self):
        nn.init.xavier_uniform_(self.W1_basis)
        nn.init.xavier_uniform_(self.W2_basis)
    ...
    def forward(self, x):
        ...
        W1 = torch.einsum('lb,bio->lio', coef1, self.W1_basis)   # [L, D, D_ff]
        W2 = torch.einsum('lb,boj->loj', coef2, self.W2_basis)   # [L, D_ff, D]
        ...
        # FFN 计算 (无门控)
        h = torch.einsum('bld,ldf->blf', x, W1_norm)

        h = F.normalize(h, p=2, dim=-1)
        h = self.scale() * h
        h = F.silu(h)  # TF 中 swish(x) = x * sigmoid(x)，PyTorch 中 F.silu 等价

        out = torch.einsum('blf,lfd->bld', h, W2_norm)
        out = F.normalize(out, p=2, dim=-1)
        return out
