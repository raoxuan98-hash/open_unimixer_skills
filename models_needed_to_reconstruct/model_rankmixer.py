...
def RankMixer_simplified(hidden_states, d_model, d_ff, scope="RankMixer", is_training=True, reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        mixing_h = tf.reshape(hidden_states, [-1, l, l, d_model // l])
...
class MixerConfig:
    def __init__(self):
        self.token_num = 32
        self.d_kv = 512 #每个head的维度，head数量等于token数量
        self.num_block = 2 #层数
...
# rate_mix_embedding = mio_dense_layer(usergroup_mix_embedding, 4, tf.nn.sigmoid, name='usergroup_mix_mix_mlp')
colossus_feature_embedding =  config.get_extra_param("colossus_feature", size=12, **get_predict_param())
# rate_mix_embedding = mio_dense_layer(colossus_feature_embedding, 8, tf.nn.sigmoid, name='colossus_feature_mlp')
...
# flattened_features = tf.reshape(reshaped_features, [-1, chunk_dim_per_token])
# flat_hidden_states = mio_dense_layer(
#     flattened_features, 
#     mixer_config.d_kv, 
#     activation=None, 
...
#     flat_hidden_states, 
#     [-1, mixer_config.token_num, mixer_config.d_kv]
# )
...
with tf.variable_scope("RankMixer_backbone"):
    ...
    for i in range(mixer_config.num_block):
        hidden_states = RankMixer_simplified(
            hidden_states,
            mixer_config.d_kv,
            mixing_ffn_dim,
    ...
    # shape_list = hidden_states.get_shape()
    # flat_dim = shape_list[1] * shape_list[2]
    # rankmixer_output = tf.reshape(hidden_states, [-1, flat_dim], name="rankmixer_output_flat")
    

with tf.name_scope('model'):
    # import kai.tensorflow as kai
    # class TensorsPrintHook(kai.training.RunHookBase):
    #     def __init__(self, your_custom_tensor_dict):
    #         self.custom_tensor_dict = your_custom_tensor_dict

    #     def before_step_run(self, step_run_context):
    #         return kai.training.StepRunArgs(fetches=self.custom_tensor_dict)
    ...
    #         print(output)
    # need_print_tensor_dict = {'rankmixer_output_shape': tf.shape(rankmixer_output)}
    # kai.add_run_hook(TensorsPrintHook(need_print_tensor_dict), "tensor_print_hook")
...
if args.mode == 'train':
    ...
    #             output += "{}: {},".format(name, val)
    #         print(output)
    # need_print_tensor_dict = gate_tensors_to_monitor
    # kai.add_run_hook(TensorsPrintHook(need_print_tensor_dict), "tensor_print_hook")
