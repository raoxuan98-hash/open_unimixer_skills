...
def multi_head_hetero_attention(X, nh=5, att_emb_size=6, scope='attention', return_attn_weights=False):
    """
    多头异构注意力机制
    Args:
        X: 输入 [B, T, D]
    ...
    Returns:
        outputs: [B, T, D]
        att_scores: [nh, B, T, T] (如果return_attn_weights=True)
    """
    with tf.variable_scope(scope):
        D = X.get_shape().as_list()[-1]
        T = X.get_shape().as_list()[1]

        Q = tf.get_variable('q_trans_matrix', shape=(T, D, att_emb_size * nh))
        K = tf.get_variable('k_trans_matrix', shape=(T, D, att_emb_size * nh))
        V = tf.get_variable("v_trans_matrix", shape=(T, D, D * nh))
        O = tf.get_variable("o_trans_matrix", shape=(T, D * nh, D))

        # token-specific 投影矩阵
        querys_0 = tf.einsum('btd,tde->bte', X, Q)
        keys_0   = tf.einsum('btd,tde->bte', X, K)
        values_0 = tf.einsum('bni, nio -> bno', X, V)

        querys = tf.stack(tf.split(querys_0, nh, axis=2))
        keys = tf.stack(tf.split(keys_0, nh, axis=2))
        values = tf.stack(tf.split(values_0, nh, axis=2))

        inner_product = tf.matmul(querys, keys, transpose_b=True) / tf.sqrt(float(att_emb_size))
        att_scores = tf.nn.softmax(inner_product)
    
        result_1 = tf.matmul(att_scores, values)
        result_2 = tf.transpose(result_1, perm=[1, 2, 0, 3])
        result = tf.reshape(result_2, (-1, T, nh * D))
        outputs = tf.einsum('bni, nio -> bno', result, O)
...
def RankMixer_simplified(hidden_states, d_model, d_ff, scope="RankMixer", is_training=True, reuse=None):
    """
    RankMixer简化版，支持返回注意力权重和输入用于监控
    """
    with tf.variable_scope(scope, reuse=reuse):
        l = hidden_states.get_shape().as_list()[1]
        
        # 保存输入用于监控token范数
        input_x_for_monitor = hidden_states

        #--------------------------Mixing Start----------------------------
        # 调用multi_head_hetero_attention，返回注意力权重
        mixing_h_hmm, att_scores = multi_head_hetero_attention(
            hidden_states, nh=4, att_emb_size=d_model//4, scope='attention', return_attn_weights=True
        )
        ...
        return final_hidden_states, mixing_h_hmm, att_scores, input_x_for_monitor
...
def compute_attention_js_diversity_single(att_scores):
    """
    计算单个注意力矩阵的行间平均JS散度
    Args:
        att_scores: [T, T] 注意力矩阵（已softmax），每行和为1
    Returns:
        float: 行间平均JS散度
    """
    T = att_scores.shape[0]
    if T <= 1:
        return 0.0
    ...
    avg_js = js_sum / pair_count if pair_count > 0 else 0.0
    return avg_js


def compute_row_entropy(att_scores):
    """
    计算注意力矩阵每一行的熵
    Args:
        att_scores: [T, T] 注意力矩阵（已softmax）
    Returns:
        tuple: (avg_entropy, max_entropy)
    """
    T = att_scores.shape[0]
    if T <= 1:
        return 0.0, 0.0
    ...
    eps = 1e-10
    entropies = -np.sum(att_scores * np.log(att_scores + eps), axis=1)
...
def compute_attention_js_diversity_batch(att_scores_batch, max_samples=100):
    """
    计算Batch注意力矩阵的行间平均JS散度
    Args:
        att_scores_batch: [B, T, T] 或 [T, T] 注意力矩阵
        max_samples: 最大计算样本数
    Returns:
        tuple: (avg_js_mean, avg_js_std, sample_count)
    """
    if len(att_scores_batch.shape) == 2:
        js = compute_attention_js_diversity_single(att_scores_batch)
        return js, 0.0, 1
    
    B = att_scores_batch.shape[0]
    sample_count = min(B, max_samples)
    
    js_list = []
    for b in range(sample_count):
        js = compute_attention_js_diversity_single(att_scores_batch[b])
        js_list.append(js)
    
    js_array = np.array(js_list)
    avg_js_mean = np.mean(js_array)
    avg_js_std = np.std(js_array)
    
    return avg_js_mean, avg_js_std, sample_count


def compute_token_norm_stats(X, max_samples=500):
    """
    计算每个token位置的L2范数统计量
    Args:
        X: 输入特征 [B, T, D]
        max_samples: 最大采样batch数
    Returns:
        dict: token范数统计信息
    """
    if len(X.shape) != 3:
    ...
    B, T, D = X.shape
    sample_count = min(B, max_samples)
    X_sampled = X[:sample_count]
    
    token_norms = np.linalg.norm(X_sampled, axis=2)
    token_norm_mean = np.mean(token_norms, axis=0)
    token_norm_std = np.std(token_norms, axis=0)
    token_norm_max = np.max(token_norms, axis=0)
    
    overall_mean = np.mean(token_norms)
    overall_std = np.std(token_norms)
    
    max_token_idx = int(np.argmax(token_norm_mean))
    min_token_idx = int(np.argmin(token_norm_mean))
    ...
    return {
        ...
        'min_token_idx': min_token_idx,
        'max_token_norm': float(token_norm_mean[max_token_idx]),
        'min_token_norm': float(token_norm_mean[min_token_idx]),
        'token_count': T,
        'feature_dim': D,
...
class AttScoresPrintHook(config.training.RunHookBase):
    """
    打印注意力分数到日志的Hook
    """
    def __init__(self, att_scores_tensor, name="att_scores", target_step=None, print_interval=None, first_n_steps=None, later_interval=None):
        self.att_scores = att_scores_tensor
        self.name = name
        self.target_step = target_step
        self.print_interval = print_interval
        self.first_n_steps = first_n_steps
        self.later_interval = later_interval
    
    def _get_step(self, step_run_context):
        return step_run_context.descr_list.step
    
    def _should_print(self, step):
        if self.first_n_steps is not None:
            if step < self.first_n_steps:
                return True
            elif self.later_interval is not None:
                return (step - self.first_n_steps) % self.later_interval == 0
            else:
                return False
        elif self.target_step is not None and step == self.target_step:
            return True
        elif self.print_interval is not None and step % self.print_interval == 0:
            return True
        return False
    
    def before_step_run(self, step_run_context):
        current_step = self._get_step(step_run_context)
        if self._should_print(current_step):
            return config.training.StepRunArgs(fetches={self.name: self.att_scores})
        return config.training.StepRunArgs(fetches={})
    
    def after_step_run(self, step_run_context, step_run_values):
        current_step = self._get_step(step_run_context)
        result = step_run_values.result.get(self.name)
        if result is None:
            return
        
        att_list = np.around(result, decimals=2).tolist()
        output_str = "ATT_SCORES_JSON_STEP_{}:{}".format(current_step, json.dumps(att_list))
        print(output_str, flush=True)


class UnifiedModelMonitorHook(config.training.RunHookBase):
    """
    统一的模型监测Hook，监控参数收敛性、注意力分布、token范数
    """
    def __init__(self, 
                 att_scores_tensor=None,
                 att_heads_axis=None,
                 input_tensor=None,
                 monitor_param_convergence=True,
                 monitor_js_diversity=True,
                 monitor_token_norm=False,
                 print_interval=5,
                 ema_decay=0.98,
                 max_att_samples=100,
                 max_token_samples=500,
                 js_warning_threshold=1e-6,
                 token_norm_warning_ratio=10.0,
                 block_name="block_0"):
        self.att_scores_tensor = att_scores_tensor
        self.att_heads_axis = att_heads_axis
        self.input_tensor = input_tensor
        self.monitor_param_convergence = monitor_param_convergence
        self.monitor_js_diversity = monitor_js_diversity and (att_scores_tensor is not None)
        self.monitor_token_norm = monitor_token_norm and (input_tensor is not None)
        self.print_interval = print_interval
        self.ema_decay = ema_decay
        self.max_att_samples = max_att_samples
        self.max_token_samples = max_token_samples
        self.js_warning_threshold = js_warning_threshold
        self.token_norm_warning_ratio = token_norm_warning_ratio
        self.block_name = block_name  # 用于区分不同block的监控
        
        self.ema_vars = {}
        self.prev_vars = {}
        self.initialized = False
    
    def _parse_attention_shape(self, att_scores):
        shape = att_scores.shape
        ndim = len(shape)
        
        if ndim == 2:
            return {
                'shape_type': 'single_head',
                'num_heads': 1,
                'batch_size': 1,
                'seq_len': shape[0],
            ...
            if self.att_heads_axis == 0:
                return {
                    'shape_type': 'heads_first',
                    'num_heads': dim0,
                    'batch_size': dim1,
                    'seq_len': dim2,
                    'original_shape': shape
                }
            elif self.att_heads_axis == 1:
                return {
                    'shape_type': 'heads_second',
                    'num_heads': dim1,
                    'batch_size': dim0,
                    'seq_len': dim2,
                    'original_shape': shape
                ...
                raise ValueError(f"注意力矩阵维度为{shape}（4维），但att_heads_axis未指定。")
        
        raise ValueError(f"不支持的注意力矩阵维度: {shape}")
    
    def _reshape_to_batch_head_matrix(self, att_scores, shape_info):
        shape_type = shape_info['shape_type']
        num_heads = shape_info['num_heads']
        batch_size = shape_info['batch_size']
        seq_len = shape_info['seq_len']
        
        sample_batch = min(batch_size, self.max_att_samples)
        
        if shape_type == 'single_head':
            if len(att_scores.shape) == 2:
                return att_scores[np.newaxis, :, :], 1
            else:
                return att_scores[:sample_batch], sample_batch
        
        elif shape_type == 'heads_first':
            att_sampled = att_scores[:, :sample_batch, :, :]
            att_reshaped = att_sampled.reshape(-1, seq_len, seq_len)
            return att_reshaped, num_heads * sample_batch
        
        elif shape_type == 'heads_second':
            att_sampled = att_scores[:sample_batch, :, :, :]
            att_reshaped = att_sampled.reshape(-1, seq_len, seq_len)
            return att_reshaped, num_heads * sample_batch
    ...
    def _compute_js_diversity_for_all_heads(self, att_scores):
        shape_info = self._parse_attention_shape(att_scores)
        if shape_info is None:
            return None
        
        att_reshaped, num_matrix = self._reshape_to_batch_head_matrix(att_scores, shape_info)
        if att_reshaped is None:
            return None
        ...
        for i in range(num_matrix):
            js = compute_attention_js_diversity_single(att_reshaped[i])
            js_list.append(js)
            
            avg_ent, _ = compute_row_entropy(att_reshaped[i])
            entropy_list.append(avg_ent)
        
        js_array = np.array(js_list)
        entropy_array = np.array(entropy_list)
        
        result = {
            'js_mean': float(np.mean(js_array)),
            'js_std': float(np.std(js_array)),
            'entropy_mean': float(np.mean(entropy_array)),
            'entropy_std': float(np.std(entropy_array)),
            'entropy_ratio': float(np.mean(entropy_array) / max_entropy),
            'max_entropy': float(max_entropy),
            'num_matrix': num_matrix,
            'shape_info': shape_info
        }
        
        if shape_info['num_heads'] > 1:
            num_heads = shape_info['num_heads']
            batch_per_head = num_matrix // num_heads
            js_per_head = []
            entropy_per_head = []
            for h in range(num_heads):
                start_idx = h * batch_per_head
                end_idx = (h + 1) * batch_per_head
                head_js_mean = float(np.mean(js_array[start_idx:end_idx]))
                head_entropy_mean = float(np.mean(entropy_array[start_idx:end_idx]))
                js_per_head.append(head_js_mean)
                entropy_per_head.append(head_entropy_mean)
            result['js_per_head'] = js_per_head
            result['entropy_per_head'] = entropy_per_head
        
        return result
    
    def before_step_run(self, step_run_context):
        step = step_run_context.descr_list.step
        ...
        if self.monitor_param_convergence:
            trainable_vars = tf.trainable_variables()
            fetches['vars'] = {v.name: v for v in trainable_vars}
        
        if self.monitor_js_diversity:
            if self.att_scores_tensor is not None:
                fetches['att_scores'] = self.att_scores_tensor
        
        if self.monitor_token_norm:
            if self.input_tensor is not None:
                fetches['input_x'] = self.input_tensor
        
        return config.training.StepRunArgs(fetches=fetches)
    
    def after_step_run(self, step_run_context, step_run_values):
        result = step_run_values.result
        step = int(step_run_context.descr_list.step)
        
        if self.monitor_param_convergence and 'vars' in result:
            self._log_param_convergence(step, result['vars'])
        
        if self.monitor_js_diversity and 'att_scores' in result:
            att_scores = result['att_scores']
            self._log_js_diversity(step, att_scores)
        
        if self.monitor_token_norm and 'input_x' in result:
            input_x = result['input_x']
            self._log_token_norm(step, input_x)
    
    def _log_param_convergence(self, step, current_vars):
        if not self.initialized:
            for name, var_val in current_vars.items():
                if isinstance(var_val, np.ndarray):
                    self.ema_vars[name] = var_val.copy()
                    self.prev_vars[name] = var_val.copy()
        ...
        for name, var_val in current_vars.items():
            ...
            if name not in self.ema_vars:
                self.ema_vars[name] = var_val.copy()
                self.prev_vars[name] = var_val.copy()
                continue
            
            ema_diff = np.linalg.norm(var_val - self.ema_vars[name])
            total_ema_diff += ema_diff
            
            update_norm = np.linalg.norm(var_val - self.prev_vars[name])
            total_update_norm += update_norm
            
            var_norm = np.linalg.norm(var_val)
            total_var_norm += var_norm
            
            self.ema_vars[name] = (self.ema_decay * self.ema_vars[name] + 
                                   (1 - self.ema_decay) * var_val)
            self.prev_vars[name] = var_val.copy()
            var_count += 1
        ...
        avg_ema_diff = total_ema_diff / var_count
        avg_update = total_update_norm / var_count
        relative_ema_diff = total_ema_diff / (total_var_norm + 1e-10)
        relative_update = total_update_norm / (total_var_norm + 1e-10)
        
        output = {
            'step': step,
            'type': 'PARAM_CONVERGENCE',
            'block': self.block_name,
            'avg_ema_diff': round(float(avg_ema_diff), 6),
            'avg_update_norm': round(float(avg_update), 6),
            'relative_ema_diff': round(float(relative_ema_diff), 6),
            'relative_update': round(float(relative_update), 6),
            'var_count': var_count
        }
        print(json.dumps(output), flush=True)
    
    def _log_js_diversity(self, step, att_scores):
        js_result = self._compute_js_diversity_for_all_heads(att_scores)
        if js_result is None:
            return
        
        output = {
            'step': step,
            'type': 'ATT_JS_DIVERSITY',
            'block': self.block_name,
            'js_mean': round(js_result['js_mean'], 6),
            'js_std': round(js_result['js_std'], 6),
            'entropy_mean': round(js_result['entropy_mean'], 6),
            'entropy_std': round(js_result['entropy_std'], 6),
            'entropy_ratio': round(js_result['entropy_ratio'], 4),
            'num_matrix': js_result['num_matrix']
        }
        
        shape_info = js_result['shape_info']
        output['shape_type'] = shape_info['shape_type']
        output['num_heads'] = shape_info['num_heads']
        output['batch_sampled'] = shape_info['batch_size']
        output['seq_len'] = shape_info['seq_len']
        
        if 'js_per_head' in js_result:
            output['js_per_head'] = [round(x, 4) for x in js_result['js_per_head']]
        if 'entropy_per_head' in js_result:
            output['entropy_per_head'] = [round(x, 4) for x in js_result['entropy_per_head']]
        
        print(json.dumps(output), flush=True)
    
    def _log_token_norm(self, step, input_x):
        norm_stats = compute_token_norm_stats(input_x, max_samples=self.max_token_samples)
        if norm_stats is None:
            return
        
        norm_ratio = norm_stats['max_token_norm'] / (norm_stats['min_token_norm'] + 1e-10)
        token_norms_list = [round(float(x), 2) for x in norm_stats['token_norm_mean']]
        
        output = {
            'step': step,
            'type': 'TOKEN_NORM_STATS',
            'block': self.block_name,
            'token_norms': token_norms_list,
            'overall_mean': round(norm_stats['overall_mean'], 4),
            'overall_std': round(norm_stats['overall_std'], 4),
            'max_token_idx': norm_stats['max_token_idx'],
            'max_token_norm': round(norm_stats['max_token_norm'], 4),
            'min_token_idx': norm_stats['min_token_idx'],
            'min_token_norm': round(norm_stats['min_token_norm'], 4),
            'norm_ratio': round(norm_ratio, 2),
            'token_count': norm_stats['token_count'],
            'sample_count': norm_stats['sample_count']
        }
...
with tf.variable_scope("RankMixer_backbone"):
    ...
    # 用于存储两个block的监控数据
    block_0_att_scores = None
    block_0_input_x = None
    block_1_att_scores = None
    block_1_input_x = None

    for i in range(mixer_config.num_block):
        hidden_states, mixing_h_hmm, att_scores, input_x = RankMixer_simplified(
            hidden_states,
            mixer_config.d_kv,
            mixing_ffn_dim,
        ...
        # 保存两个block的注意力分数和输入用于监控
        if i == 0:
            # block 0: 保留所有head，限制batch样本数 [nh, bs, T, T]
            if att_scores is not None:
                block_0_att_scores = att_scores[:, :100, :, :]  # [4, 100, T, T]
            if input_x is not None:
                block_0_input_x = input_x[:500, :, :]  # [500, T, D]
        elif i == 1:
            # block 1: 同样处理
            if att_scores is not None:
                block_1_att_scores = att_scores[:, :100, :, :]  # [4, 100, T, T]
            if input_x is not None:
                block_1_input_x = input_x[:500, :, :]  # [500, T, D]
        # mixer_regloss += mixer_config.alpha * load_balance_loss
    
    # 添加模型监测Hook - Block 0
    if block_0_att_scores is not None:
        config.add_run_hook(
            UnifiedModelMonitorHook(
                att_scores_tensor=block_0_att_scores,
                att_heads_axis=0,  # head在第0维 [nh, bs, T, T]
                input_tensor=block_0_input_x,
                monitor_param_convergence=True,
                monitor_js_diversity=True,
                monitor_token_norm=True,
                print_interval=5,
                ema_decay=0.95,
                max_att_samples=100,  # 与切片保持一致
                max_token_samples=500,
                js_warning_threshold=1e-7,
                token_norm_warning_ratio=5.0,
                block_name="block_0"
            ),
            "unified_model_monitor_hook_block_0"
        )
        
        # 打印Hook只取第一个head第一个样本用于可视化
        config.add_run_hook(
            AttScoresPrintHook(
                block_0_att_scores[0, 0, :, :],  # [T, T] 只取第一个head和第一个样本
                name="att_scores_block_0",
                print_interval=50
            ),
            "att_scores_print_hook_block_0"
        )
    
    # 添加模型监测Hook - Block 1 (第二个模块)
    if block_1_att_scores is not None:
        config.add_run_hook(
            UnifiedModelMonitorHook(
                att_scores_tensor=block_1_att_scores,
                att_heads_axis=0,  # head在第0维 [nh, bs, T, T]
                input_tensor=block_1_input_x,
                monitor_param_convergence=True,
                monitor_js_diversity=True,
                monitor_token_norm=True,
                print_interval=5,
            ...
            "unified_model_monitor_hook_block_1"
        )
        
        # 打印Hook只取第一个head第一个样本用于可视化
        config.add_run_hook(
            AttScoresPrintHook(
                block_1_att_scores[0, 0, :, :],  # [T, T] 只取第一个head和第一个样本
                name="att_scores_block_1",
                print_interval=50
            ),
            "att_scores_print_hook_block_1"
        )
