...
def self_weighted_attention(query_input, basis_num=2, att_emb_size=3, scope='attention'):
    with tf.variable_scope(scope):
        D = query_input.get_shape().as_list()[-1]
        T = query_input.get_shape().as_list()[1]

        W_1 = tf.get_variable('W_1', shape=[T, basis_num])
        W_2 = tf.get_variable('W_2', shape=[T, basis_num])

        Q = tf.get_variable('q_trans_matrix', shape=(basis_num, D, att_emb_size))
        K = tf.get_variable('k_trans_matrix', shape=(basis_num, D, att_emb_size))
        W_logits = tf.get_variable("Matrices_Wi", shape=(T, D, D))

        # ===== basis 生成 =====
        # 1. 生成 token-dependent Q/K/V
        Q_tilde = tf.einsum('tk,kde->tde', W_1, Q)  # [T,D,E]
        K_tilde = tf.einsum('tk,kde->tde', W_2, K)

        # 2. token 使用自己的投影矩阵
        querys = tf.einsum('btd,tde->bte', query_input, Q_tilde)
        keys   = tf.einsum('btd,tde->bte', query_input, K_tilde)
        values = tf.einsum('bni, nio -> bno', query_input, W_logits)
...
def UniMixer(X, basis_num=4, att_emb_size=5, sinkhorn_iters=5, scope='attention', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        D = X.get_shape().as_list()[-1]
        T = X.get_shape().as_list()[1]
        # token embedding
        token_embeddings = tf.get_variable('token_embeddings', dtype=tf.float32, shape=[T, D])

        Q_all_token_outputs = []
        K_all_token_outputs = []
        for i in range(T):
            token_slice = tf.slice(token_embeddings, [i, 0], [1, -1])  # [B, 1, D]
            Q_weight_output = mio_dense_layer(token_slice, units=basis_num, activation=None, name=f"Q_meta_map_{i}")
            K_weight_output = mio_dense_layer(token_slice, units=basis_num, activation=None, name=f"K_meta_map_{i}")
            Q_all_token_outputs.append(Q_weight_output)
            K_all_token_outputs.append(K_weight_output)

        s_q_output = tf.concat(Q_all_token_outputs, axis=0)
        s_k_output = tf.concat(K_all_token_outputs, axis=0)
        ...
        K = tf.get_variable('k_trans_matrix', shape=(basis_num, D, att_emb_size))
        W_logits = tf.get_variable("Matrices_Wi", shape=(T, D, D))
        A_logits = tf.get_variable("Matrices_A", shape=(T, T))
        lambda_logits = tf.get_variable("lambda_logits", shape=(T, T))
        lambda_weight = tf.nn.sigmoid(lambda_logits)
        W_logits_constraint = (W_logits + tf.transpose(W_logits, perm=[0, 2, 1])) * 0.5
        # A_logits_constraint = (A_logits + tf.transpose(A_logits, perm=[0, 2, 1])) * 0.5
        W_V = log_sinkhorn(W_logits_constraint, n_iters=sinkhorn_iters, temperature=1)
        ...
        # 2. token 使用自己的投影矩阵
        querys = tf.einsum('btd,tde->bte', X, Q_tilde)
        keys   = tf.einsum('btd,tde->bte', X, K_tilde)
        values = tf.einsum('bni, nio -> bno', X, W_V)

        inner_product = tf.matmul(querys, keys, transpose_b=True) / tf.sqrt(float(att_emb_size))

        att_scores = tf.nn.softmax(inner_product)
        # att_scores = (att_scores + tf.transpose(att_scores, perm=[0, 2, 1])) * 0.5
        A_expand = tf.expand_dims(A_logits, axis=0)   
        lambda_weight_expand = tf.expand_dims(lambda_weight, axis=0)  
        att_scores = lambda_weight_expand * A_expand + (1 - lambda_weight_expand) * att_scores
        att_scores = (att_scores + tf.transpose(att_scores, perm=[0, 2, 1])) * 0.5
        att_scores = log_sinkhorn(att_scores, n_iters=sinkhorn_iters, temperature=1)
...
def RankMixer_simplified(hidden_states, d_model, d_ff, scope="RankMixer", is_training=True, reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # 输入hidden_states:[Batchsize, T, D]
        mixing_h = tf.reshape(hidden_states, [-1, l * d_model])
        N = l * l          # Chunks
        K = d_model // l         # Chunk dim
        mixing_h = tf.reshape(mixing_h, [-1, N, K])
        #--------------------------Mixing Start----------------------------
        mixing_h_hmm = UniMixer(mixing_h, basis_num=8, att_emb_size=l//2)
        mixing_h = mixing_h_hmm
        mixing_h = tf.reshape(mixing_h, [-1, l, d_model])
        #--------------------------Mixing End----------------------------
        # 输出mixing_h:[Batchsize, T, D]
...
class MixerConfig:
    def __init__(self):
        self.token_num = 16 #origin = 32
        self.d_kv = 1024 #origin = 512 每个head的维度，head数量等于token数量
        self.num_block = 2 #层数
...
#监控
# rta_app_list_length = config.get_extra_param('rta_app_list_length', size=1, reversed=False, default_value=0.0)
# d_app_list_length = config.get_extra_param('d_app_list_length', size=1, reversed=False, default_value=0.0)
...
if args.mode == 'train':
    ...
    # need_print_tensor_dict["D_hmm"] = D_hmm
    # need_print_tensor_dict["T_hmm"] = T_hmm
    # need_print_tensor_dict["normalized_att_scores_hmm"] = tf.shape(normalized_att_scores_hmm)
    # need_print_tensor_dict["query_input_hmm"] = tf.shape(query_input_hmm)
