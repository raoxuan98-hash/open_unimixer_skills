...
def normalized_ffn_swish_glu(hidden_states,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # weights: (in, out)
        w1 = tf.get_variable("w1", [d_model, d_ff], initializer=tf.random_normal_initializer())
        w2 = tf.get_variable("w2", [d_model, d_ff], initializer=tf.random_normal_initializer())
        w3 = tf.get_variable("w3", [d_ff, d_model], initializer=tf.random_normal_initializer())
        
        # scale params
        u_scale = tf.get_variable("u_scale", [d_ff], initializer=tf.constant_initializer(0.5413))
        v_scale = tf.get_variable("v_scale", [d_ff], initializer=tf.constant_initializer(0.5413))
        
        # normalize weights across input dimension (axis=0 for [in, out])
        nw1 = tf.nn.l2_normalize(w1, axis=0)
        nw2 = tf.nn.l2_normalize(w2, axis=0)
        nw3 = tf.nn.l2_normalize(w3, axis=0)
        
        u = tf.matmul(reshaped_input, nw1)
        v = tf.matmul(reshaped_input, nw2)
        
        # activation normalization
        u = tf.nn.l2_normalize(u, axis=-1)
        v = tf.nn.l2_normalize(v, axis=-1)
        
        scaled_u = tf.nn.softplus(u_scale) * u
        scaled_v = tf.nn.softplus(v_scale) * v * math.sqrt(d_model)
        
        # swiglu activation
        h = swish(scaled_v) * scaled_u
        
        # down projection and output normalization
        out = tf.matmul(h, nw3)
        out = tf.nn.l2_normalize(out, axis=-1)
    
        output_states = tf.expand_dims(out, axis=1)
        return output_states
...
def Normalized_TokenMixer(hidden_states, d_model, d_ff, scope="Normalized_TokenMixer", denseFFN=False, is_training=True, reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        b, t, d = hidden_states.get_shape().as_list()
        l = t

        alpha1 = tf.get_variable("alpha1", [d], initializer=tf.constant_initializer(-2.9706))
        alpha2 = tf.get_variable("alpha2", [d], initializer=tf.constant_initializer(-2.9706))

        mixing_h = tf.reshape(hidden_states, (-1, t, l, d // l))
        ...
        for i in range(l):
            ...
            with tf.variable_scope(f"TokenMixer_1_{i}_FFN"): # 建议区分scope
                token_output = normalized_ffn_swish_glu(
                    token_slice, middle_dim, d_ff, 
                    scope="l_ffn"
                )
            
            all_token_outputs.append(token_output)

        output_1 = tf.concat(all_token_outputs, axis=1)
        output_1 = tf.nn.l2_normalize(output_1, axis=-1)
        
        # Revert
        revert_h = tf.reshape(output_1, (-1, l, t, middle_dim // t))
        revert_h = tf.transpose(revert_h, perm=[0, 2, 1, 3])
        reverted_output = tf.reshape(revert_h, (-1, t, d))
        reverted_output = tf.nn.l2_normalize(reverted_output, axis=-1)
        
        hidden_states = hidden_states + tf.nn.softplus(alpha1) * reverted_output
        hidden_states = tf.nn.l2_normalize(hidden_states, axis=-1)

        all_token_outputs = []

        for i in range(t):
            token_slice = tf.slice(hidden_states, [0, i, 0], [-1, 1, -1])
            with tf.variable_scope(f"TokenMixer_2_{i}_FFN"): # 建议区分scope
                token_output = normalized_ffn_swish_glu(
                    token_slice, d_model, d_ff, 
                    scope="t_ffn"
                )
            
            all_token_outputs.append(token_output)

        output_2 = tf.concat(all_token_outputs, axis=1)
        output_2 = tf.nn.l2_normalize(output_2, axis=-1)
        
        final_hidden_states = hidden_states + tf.nn.softplus(alpha2) * output_2
        final_hidden_states = tf.nn.l2_normalize(final_hidden_states, axis=-1)
...
with tf.variable_scope("TokenMixer_backbone"):
    mixing_ffn_dim = mixer_config.d_kv * mixer_config.d_ff_coefficient
    mixer_regloss = 0

    for i in range(mixer_config.num_block):
        hidden_states= Normalized_TokenMixer(
            hidden_states,
            mixer_config.d_kv,
            mixing_ffn_dim,
            scope=f"Normalized_block_{i}",
            is_training=is_training
        )

    tokenmixer_output = tf.reduce_mean(hidden_states, axis=1, name="tokenmixer_mean_pooling_output")
    rankmixer_output = tf.nn.l2_normalize(tokenmixer_output, axis=-1)
